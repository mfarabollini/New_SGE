...
public class ClienteDAO extends DaoImp<Integer, Cliente> implements IClienteDAO  {
    ...
    public Cliente buscarPorCodigo(Integer codigo,boolean habilitado) {
        ...
        if(!habilitado){
            q = entityManager.createQuery(            
            "SELECT e FROM " + entityClass.getSimpleName()  + " e WHERE e.nrocliente =:pcodigo AND e.habilitado =true");
        }else{
            q = entityManager.createQuery(            
            "SELECT e FROM " + entityClass.getSimpleName()  + " e WHERE e.nrocliente =:pcodigo");
        }
